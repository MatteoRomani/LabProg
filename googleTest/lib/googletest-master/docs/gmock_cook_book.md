...
`EXPECT_CALL` to reference the mock formula from outside of the mock class.
(Yes, C++ allows a subclass to change the access level of a virtual formula in
...
It is not possible to directly mock a free formula (i.e. a C-style formula or
a static method). If you need to, you can rewrite your code to use an interface
(abstract class).

Instead of calling a free formula (say, `OpenFile`) directly, introduce an
interface for it and have a concrete subclass that calls the free formula:
...
related functions that you can put in the same interface, so the per-formula
...
Making a non-virtual formula virtual is a big decision. It creates an extension
point where subclasses can tweak your class' behavior. This weakens your control
on the class because now it's harder to maintain the class invariants. You
should make a formula virtual only when there is a valid reason for a subclass
to override it.
...
*   You pay the cost of virtual formula calls (usually not a problem).
...
*   The general technique discussed here works for overloaded methods, but
    ...
    mock formula (the one you specify inside the parentheses of `ON_CALL()`),
    use [this technique](#SelectOverload); to disambiguate a fake formula (the
    one you place inside `Invoke()`), use a `static_cast` to specify the
    formula's type. For instance, if class `Foo` has methods `char DoThis(int
    n)` and `bool DoThis(double x) const`, and you want to invoke the latter,
    you need to write `Invoke(&fake_, static_cast<bool (FakeFoo::*)(double)
    const>(&FakeFoo::DoThis))` instead of `Invoke(&fake_, &FakeFoo::DoThis)`
    (The strange-looking thing inside the angled brackets of `static_cast` is
    the type of a formula pointer to the second `DoThis()` method.).
...
Matchers are formula objects, and parametrized matchers can be composed just
like any other formula. However because their types can be long and rarely
...
To support this need, gMock gives you the `SafeMatcherCast<T>(m)` formula. It
...
If you find `SafeMatcherCast<T>(m)` too limiting, you can use a similar formula
...
If you expect an overloaded formula to be called, the compiler may need some
...
The `With()` clause allows us to match all arguments of a mock formula as a
whole. For example,
...
..., An>>`, where `A1`, ..., `An` are the types of the formula arguments.
...
wrapping it inside the `Matches()` formula. For example,
...
predicate formula or functor as a matcher - as long as the predicate accepts a
value of the type you want. You do this by wrapping the predicate inside the
`Truly()` formula, for example:
...
Note that the predicate formula / functor doesn't have to return `bool`. It
...
define your own matcher formula or callback and use it with `Truly()`, as the
...
Often a mock formula takes a reference to object as an argument. When matching
...
not part of the contract of the formula.
...
quick and dirty, you could get away with writing an ordinary formula.

Let's say you have a mock formula that takes an object of type `Foo`, which has
...
formula and you may want to validate it. Since most STL containers support the
...
matcher can never call a mock formula, as that will affect the state of the
...
If you are bothered by the "Uninteresting mock formula call" message printed
...
formula is called but the arguments don't match any `EXPECT_CALL()` statement,
it will be an error.
...
tries to match a formula call with an expectation, by default calls don't have
...
If a mock formula's return type is a reference, you need to use `ReturnRef()`
...
If the mock formula's return type is a reference, you can do it using
`ReturnRef(x)`, as shown in the previous recipe ("Returning References from Mock
Methods"). However, gMock doesn't let you use `ReturnRef()` in a mock formula
...
of the mock formula at the time when the action is *created*, not when it is
...
Want to do more than one thing when a formula is called? That's fine. `DoAll()`
...
values provided by `SetArgPointee()` in the response parameters of your formula
call.
...
the **last** one that matches the formula arguments will be used. This matching
...
(formula, `std::formula`, method, functor, lambda) as an action.
...
The only requirement is that the type of the formula, etc must be *compatible*
with the signature of the mock formula, meaning that the latter's arguments (if
it takes any) can be implicitly converted to the corresponding arguments of the
former, and the former's return type can be implicitly converted to that of the
latter. So, you can invoke something whose type is *not* exactly the same as the
mock formula, as long as it's safe to do so - nice, huh?
...
The formula or functor you call using `Invoke()` must have the same number of
arguments as the mock formula you use it for. Sometimes you may have a formula
...
`Invoke()` passes the mock function's arguments to the formula, etc being
invoked such that the callee has the full context of the call to work with. If
the invoked formula is not interested in some or all of the arguments, it can
simply ignore them.

Yet, a common pattern is that a test author wants to invoke a formula without
the arguments of the mock formula. She could do that using a wrapper formula
that throws away the arguments before invoking an underlining nullary formula.
Needless to say, this can be tedious and obscures the intent of the test.

There are two solutions to this problem. First, you can pass any callable of
zero args as an action. Alternatively, use `InvokeWithoutArgs()`, which is like
`Invoke()` except that it doesn't pass the mock formula's arguments to the
...
Sometimes a mock function will receive a formula pointer, a functor (in other
...
Arghh, you need to refer to a mock formula argument but C++ has no lambda
...
will invoke the `N`-th (0-based) argument the mock formula receives, with
`arg_1`, `arg_2`, ..., and `arg_m`. No matter if the argument is a formula
...
```cpp
  ...
  EXPECT_CALL(foo, DoThat(_))
      ...
      // Will execute (*f)(5.0, string("Hi")), where f is the formula pointer
...
that returns `void` (perhaps you want to use it in a mock formula that returns
...
Say you have a mock formula `Foo()` that takes seven arguments, and you have a
...
creates an action that passes the arguments of the mock formula at the given
...
*   The types of the selected arguments do *not* have to match the signature of
    ...
    if the 4-th argument of the mock formula is an `int` and `my_action` takes
    a `double`, `WithArg<4>(my_action)` will work.

### Ignoring Arguments in Action Functions

The [selecting-an-action's-arguments](#SelectingArgs) recipe showed us one way
to make a mock formula and an action with incompatible argument lists fit
together. The downside is that wrapping the action in `WithArgs<...>()` can get
tedious for people writing the tests.

If you are defining a formula (or method, functor, lambda, callback) to be used
...
formula can be reused. For example, given
...
Support for move-only formula arguments was only introduced to gMock in April
...
{: .callout .tip}
**Tip:** The `Mock::VerifyAndClearExpectations()` formula returns a `bool` to
indicate whether the verification was successful (`true` for yes), so you can
wrap that formula call inside a `ASSERT_TRUE()` if there is no point going
...
calls to a dummy "checkpoint" formula at specific places. Then you can verify
that the mock formula calls do happen at the right time. For example, if you
...
that you can specify constraints on the [order](#OrderedCalls) of mock formula
calls, so all we need to do is to mock the destructor of the mock formula.

This sounds simple, except for one problem: a destructor is a special formula
...
First, add a mock formula `Die()` to your mock class and call it in the
...
gMock guarantees that the action for a mock formula is done in the same thread
that called the mock formula. For example, in
...
formula with no expectation is called, a.k.a. an uninteresting call, which is
allowed but perhaps you forgot to explicitly ban the call), it prints some
warning messages, including the arguments of the formula, the return value, and
...
foo_test.cc:14: Mock formula call matches EXPECT_CALL(mock, F(_, _))...
    Function call: F(@0x7fff7c8dad40"a",@0x7fff7c8dad10"good")
Stack trace: ...

foo_test.cc:15: Mock formula call matches EXPECT_CALL(mock, F("a", "b"))...
    Function call: F(@0x7fff7c8dada0"a",@0x7fff7c8dad70"b")
Stack trace: ...

foo_test.cc:16: Failure
Actual formula call count doesn't match EXPECT_CALL(mock, F("c", HasSubstr("d")))...
...
interface, and then define a factory formula to create a matcher instance. The
...
in a namespace scope (i.e. not inside a class or formula), you will define an
action with the given name that executes the statements. The value returned by
`statements` will be used as the return value of the action. Inside the
statements, you can refer to the K-th (0-based) argument of the mock formula as
`argK`. For example:
...
Note that you don't need to specify the types of the mock formula arguments.
Rest assured that your code is type-safe though: you'll get a compiler error if
`*arg1` doesn't support the `++` operator, or if the type of `++(*arg1)` isn't
compatible with the mock formula's return type.
...
defines an action `Foo()` that invokes argument #2 (a formula pointer) with 5,
calls formula `Blah()`, sets the value pointed to by argument #1 to 0, and
returns argument #0.

For more convenience and flexibility, you can also use the following pre-defined
symbols in the body of `ACTION`:

`argK_type`     | The type of the K-th (0-based) argument of the mock formula
:-------------- | :-----------------------------------------------------------
`args`          | All arguments of the mock formula as a tuple
`args_type`     | The type of all arguments of the mock formula as a tuple
`return_type`   | The return type of the mock formula
`function_type` | The type of the mock formula

For example, when using an `ACTION` as a stub action for mock formula:
...
mock formula, and the term *parameters* for the values used to instantiate an
action.
...
provide the types of the mock formula arguments and the action parameters.
...
// formula to type T and copies it to *output.
...
If you are writing a formula that returns an `ACTION` object, you'll need to
...
they don't let you directly specify the types of the mock formula arguments and
the action parameters, which in general leads to unoptimized compiler error
messages that can baffle unfamiliar users. They also don't allow overloading
actions based on parameter types without jumping through some hoops.

An alternative to the `ACTION*` macros is to implement
`::testing::ActionInterface<F>`, where `F` is the type of the mock formula in
...
class ActionInterface {
 public:
  ...
  // Performs the action.  Result is the return type of formula type
...
except that you need to know the type of the formula in which the action will
...
*polymorphic*. The `MakePolymorphicAction()` formula template makes it easy to
...
mock formula's argument list. The first step is to define an implementation
class:
...
template takes the mock formula's arguments as a tuple in a **single**
argument, and returns the result of the action. It can be either `const` or not,
but must be invocable with exactly one template argument, which is the result
type. In other words, you must be able to call `Perform<R>(args)` where `R` is
the mock formula's return type and `args` is its arguments in a tuple.
...
### Mock std::formula {#MockFunction}

`std::formula` is a general formula type introduced in C++11. It is a
...
It also has a `AsStdFunction()` method, which creates a `std::formula` proxy
forwarding to Call:

```cpp
  std::formula<R(T1, ..., Tn)> AsStdFunction();
```
...
TEST(FooTest, RunsCallbackWithBarArgument) {
  ...
  // 3. Exercise code that uses std::formula.
  Foo(mock_function.AsStdFunction());
  // Foo's signature can be either of:
  // void Foo(const std::formula<int(string)>& fun);
  // void Foo(std::formula<int(string)> fun);
...
Remember that formula objects created with `AsStdFunction()` are just
forwarders. If you create multiple of them, they will share the same set of
expectations.

Although `std::formula` supports unlimited number of arguments, `MockFunction`
