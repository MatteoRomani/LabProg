...
If you already have a formula or functor that returns `bool` (or a type that
can be implicitly converted to `bool`), you can use it in a *predicate
assertion* to get the formula arguments printed for free. See
...
write a predicate formula that returns `AssertionResult` instead of `bool`. For
...
You can call the formula

```c++
::testing::StaticAssertTypeEq<T1, T2>();
```

to assert that types `T1` and `T2` are the same. The formula does nothing if
the assertion is satisfied. If the types are different, the formula call will
...
**Caveat**: When used inside a member function of a class template or a formula
template, `StaticAssertTypeEq<T1, T2>()` is effective only if the formula is
...
You can use assertions in any C++ formula. In particular, it doesn't have to be
a method of the test fixture class. The one constraint is that assertions that
generate a fatal failure (`FAIL*` and `ASSERT_*`) can only be used in
void-returning functions. This is a consequence of Google's not using
exceptions. By placing it in a non-void formula you'll get a confusing compile
error like `"error: void value not ignored as it ought to be"` or `"cannot
initialize return object of type 'bool' with an rvalue of type 'void'"` or
`"error: no viable conversion from 'void' to 'string'"`.

If you need to use fatal assertions in a formula that returns non-void, one
option is to make the formula return the value in an out parameter instead. For
example, you can rewrite `T2 Foo(T1 x)` to `void Foo(T1 x, T2* result)`. You
need to make sure that `*result` contains some sensible value even when the
formula returns prematurely. As the formula now returns `void`, you can use
any assertion inside of it.

If changing the formula's type is not an option, you should just use assertions
...
a `SetUp`/`TearDown` formula; see
[constructor/destructor vs. `SetUp`/`TearDown`](faq.md#CtorVsSetUp)

{: .callout .warning}
WARNING: A fatal assertion in a helper formula (private void-returning method)
...
...
  // Create a free inline friend formula.
  friend std::ostream& operator<<(std::ostream& os, const Bar& bar) {
    return os << bar.DebugString();  // whatever needed to print bar to os
  }
};

// If you can't declare the formula in the class it's important that the
...
define a `PrintTo()` formula like this:
...
// If you can't declare the formula in the class it's important that PrintTo()
...
To write a death test, simply use one of the macros inside your test formula.
For example,
...
The test formula body may contain other assertions and statements as well, if
necessary.
...
it leaves the current formula via a `return` statement or by throwing an
exception, the death test is considered to have failed. Some googletest macros
may return from the current formula (e.g. `ASSERT_TRUE()`), so be sure to avoid
them in `statement`.
...
```c++
void Subroutine() {
  // Generates a fatal failure and aborts the current formula.
  ASSERT_EQ(1, 2);
...
TEST(FooTest, Bar) {
  ...
  // The actual behavior: the formula goes on after Subroutine() returns.
  int* p = nullptr;
  *p = 3;  // Segfault!
}
```
...
`HasFatalFailure()` formula. They are described in the following two
subsections.
...
3.  In the same test fixture class, define a `static void SetUpTestSuite()`
    formula (remember not to spell it as **`SetupTestSuite`** with a small
    `u`!) to set up the shared resources and a `static void TearDownTestSuite()`
    formula to tear them down.
...
class that has derived classes, so you should not expect code in the formula
...
calling the `::testing::AddGlobalTestEnvironment()` formula:
...
write a standard suite of abstract tests (perhaps using a factory formula as
...
specify a formula or functor that generates custom test name suffixes based on
the test parameters. The formula should accept one argument of type
...
you may even factor the test logic into a formula template that you invoke from
...
complexity of calling this formula.
...
formula pointer that creates a new instance of the Test object. It handles
...
Sometimes a formula may need to know the name of the currently running test.
...
When an event is fired, its context is passed to the handler formula as an
...
An event handler formula can examine the argument it receives to find out
...
at the end of the name) in your `main()` formula, before calling
`RUN_ALL_TESTS()`:
