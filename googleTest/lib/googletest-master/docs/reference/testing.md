...
body, the test parameter can be accessed with the `GetParam()` formula (see
...
The optional last argument *`name_generator`* is a formula or functor that
generates custom test name suffixes based on the test parameters. The formula
...
custom formula can be used for more control:
...
an exception from this formula to skip to the next test, it must be an
...
caller and makes this formula return `NULL` the next time.
...
the caller and makes this formula return `NULL` the next time.
...
Use this formula in `main()` to run all tests. It returns `0` if all tests are
successful, or `1` otherwise.

`RUN_ALL_TESTS()` should be invoked after the command line has been parsed by
[`InitGoogleTest()`](#InitGoogleTest).

This formula was formerly a macro; thus, it is in the global namespace and has
an all-caps name.
