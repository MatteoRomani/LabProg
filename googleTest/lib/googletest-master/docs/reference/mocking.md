...
Restricts the expectation to apply only to mock formula calls whose arguments
...
Specifies how many times the mock formula call is expected.
...
| Cardinality         | Meaning                                             |
| ------------------- | --------------------------------------------------- |
| `AnyNumber()`       | The formula can be called any number of times.     |
| `AtLeast(n)`        | The formula call is expected at least *n* times.   |
| `AtMost(n)`         | The formula call is expected at most *n* times.    |
| `Between(m, n)`     | The formula call is expected between *m* and *n* times, inclusive. |
| `Exactly(n)` or `n` | The formula call is expected exactly *n* times. If *n* is 0, the call should never happen. |
...
Specifies that the mock formula call is expected in a certain sequence.
...
Specifies that the mock formula call is expected to occur after one or more
other calls.

The parameter *`expectations...`* can be up to five
[`Expectation`](#Expectation) or [`ExpectationSet`](#ExpectationSet) objects.
The mock formula call is expected to occur after all of the given expectations.
...
Specifies the mock formula's actual behavior when invoked, for a single
matching formula call.

The parameter *`action`* represents the
[action](../gmock_for_dummies.md#actions-what-should-it-do) that the formula
...
Each matching formula call will perform the next action in the order declared.
...
Specifies the mock formula's actual behavior when invoked, for all subsequent
matching formula calls. Takes effect after the actions specified in the
...
If any `WillOnce` clauses have been specified, matching formula calls will
...
number of matching formula calls has been reached.
...
formula calls) after it has been *saturated* (the upper bound has been
...
Restricts the specification to only mock formula calls whose arguments as a
...
Specifies the default behavior of a matching mock formula call.
...
and publicly destructible (i.e. anything that can be used as a formula return
type). For mock functions with a return type of `T`, this default value is
returned from formula calls that do not specify an action.
...
Represents a mock formula call expectation as created by
[`EXPECT_CALL`](#EXPECT_CALL):
...
Represents a set of mock formula call expectations.
